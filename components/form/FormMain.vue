...
<script setup lang="ts">
  ...
  const validation = reactive(validationForm<typeof myForm>('FormMain', myForm))

  watch(() => myForm,
    (val) => Object.assign(validation, validationForm<typeof val>('FormMain', val)),
    { deep: true }
  )
  ...
  const submit = async () => {
    ...
    await $fetch('/api/sendMail', {
      ...
      body: validation.createFormDate('Заявка с FormMain'),
    })
    .then(() => {
    ...
    .catch((error) => {
      ...
      console.error(`ERRORFormMain ${error}`)
    })
