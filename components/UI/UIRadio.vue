  ...
  <div class="UIRadio d-flex fd-c">
    ...
    <div class="__list d-flex flex-wrap">
      <div
        ...
        @click="active?.key == item.key && !options?.noToggle ? (active = {} as UIRadioData) : active = item"
        >
        <span
...
<script setup lang="ts">
  ...
  const active = ref<UIRadioData>(props.data.find(i => i.key == props.options?.checked?.key) || {} as UIRadioData)
  ...
  $listen('reset', () => {
    active.value = props.data.find(i => i.key == props.options?.checked?.key) || {} as UIRadioData
  })
